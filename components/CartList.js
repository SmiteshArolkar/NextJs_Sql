  ...
  return (
    <div className="p-4">
      <div className="mb-4 flex">
        <button
          className={`mr-4 ${
            activeTab === 'pending' ? 'text-blue-300 font-bold ' : 'text-gray-500'
          }`}
          onClick={() => {
            setFilteredItems([])
        ...
        <button
          className={`mr-4 ${
            activeTab === 'approved' ? 'text-green-500 font-bold ' : 'text-gray-500'
          }`}
          onClick={() => {
            setFilteredItems([])
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredItems.map((item) => (
          ...
          >
            ...
            {
                item.status === 'pending' ? <>
                <button className='border-2 px-2 w-1/4  my-4 mx-auto py-1 m-1 bg-red-300 rounded-lg shadow-lg'>Cancel Request</button>
                </> : <>
                <div className='border-2 rounded-xl p-2 bg-yellow-200 shadow'>
                    Supplier Details
                </div>
                </>
            }
